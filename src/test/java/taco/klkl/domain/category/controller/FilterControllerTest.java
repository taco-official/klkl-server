...
public class FilterControllerTest {
	...
	public void testGetFilterWithValidQuery() throws Exception {
		...
		List<FilterWithSubcategoryResponse> mockResponse = mockSubcategoryList.stream()
		...
		mockMvc.perform(get("/v1/filters")
			...
			.andExpect(jsonPath("$.data[0].filters[0].id", is(1)))
			.andExpect(jsonPath("$.data[0].filters[0].name", is(FilterName.CONVENIENCE_STORE.getKoreanName())))
			.andExpect(jsonPath("$.data[0].filters[1].id", is(2)))
			.andExpect(jsonPath("$.data[0].filters[1].name", is(FilterName.CILANTRO.getKoreanName())))
			.andExpect(jsonPath("$.data[1].subcategoryId", is(2)))
			.andExpect(jsonPath("$.data[1].subcategory", is(SubcategoryName.SNACK.getKoreanName())))
			.andExpect(jsonPath("$.data[1].filters[0].id", is(1)))
			.andExpect(jsonPath("$.data[1].filters[0].name", is(FilterName.CONVENIENCE_STORE.getKoreanName())))
